...
def read_all_json_files_from_folder_to_df():
    ...
    all_df = pd.concat(dfs, ignore_index=True).sort_values(
        by=["execution_timestamp", "race_id", "name"]
    )
    ...
    key_values_df = all_df[
        all_df["desc"].isin(
            [
                "Viestin aikaväliennuste väärin",
                "Yksilöennusteen keskivirhe",
                "Yksilön aikaväliennuste väärin",
                "Yksilön aikavälin keskikoko",
            ]
        )
    ]  # "Aiempien Jukoloiden määrä (keskiarvo)",

    latest_kvs = key_values_df[
        key_values_df["execution_timestamp"]
        == key_values_df["execution_timestamp"].max()
    ]
    shared.log_df(
        latest_kvs.pivot(index="race_id", columns="name", values="value").round(3)
    )

    # Weighted by number of participants which is roughly legs * teams,
    # so Venlas with less legs have less influence
    averages_df = (
        key_values_df.groupby(["execution_timestamp", "desc"])
        .apply(lambda x: np.average(x.value, weights=x.num_runners))
        .reset_index()
    )
    averages_df.columns = ["execution_timestamp", "kpi", "value"]
    # convert long format to wide format
    wide_averages_df = averages_df.pivot(
        index="execution_timestamp", columns="kpi", values="value"
    )
