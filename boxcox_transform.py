...
) -> tuple[np.ndarray | pl.Series, float, float]:
...
def boxcox_and_normalize(
    values: np.ndarray | pl.Series, params: BoxCoxParams
) -> np.ndarray:
...
def inverse_normalized_and_boxcox(
    normalized_bc_values: np.ndarray, params: BoxCoxParams
) -> np.ndarray:
...
def fit_boxcox_and_normalize(
    runs_df: pl.DataFrame, history_reference_df: pl.DataFrame
    ...
    history_capped_paces = np.clip(
        history_reference_df["pace"].to_numpy(), a_min=4, a_max=40
    )
    ...
    boxcox_params = BoxCoxParams(
        lmbda=race_specific_bc_lambda, bc_mean=bc_mean, bc_std=bc_std
    )

    logging.info(f"{shared.race_id_str()} {race_specific_bc_lambda=}, {boxcox_params=}")
