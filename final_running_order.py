...
def create_running_order_from_results():
    ...
    results = pd.read_csv(
        f"data/results_with_dist_j{year}_{race_type}.tsv", delimiter="\t"
    )
    final = pd.DataFrame()
    final["team_id"] = results["team-id"]
    final["team"] = [
        f"{name} {number}"
        for name, number in zip(results["team-name"], results["team-nro"])
    ]
    ...
    final.to_csv(
        f"data/running_order_final_{shared.race_id_str()}.tsv",
        "\t",
    ...
    team_countries = (
        final[["team_id", "team_base_name", "team_country"]]
        .sort_values("team_id")
        .drop_duplicates()
    )
