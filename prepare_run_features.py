...
def combine_estimates_with_running_order():
    ...
    estimates = running_order_with_estimates.rename(
        columns={"ro_orig_name": "name", "leg_dist": "dist"}
    )
    ...
    shared.log_df(
        estimates[["num_runs", "log_std", "log_mean"]]
        .groupby("num_runs")
        .agg(["mean"])
        .round(2)
    )
...
def _load_pymc_history_values_for_running_order_names():
    ...
    runs = pd.read_csv(
        f"data/long_runs_and_running_order_{shared.race_id_str()}.tsv", delimiter="\t"
    )
    ...
    results_dir = (
        "~/koodi/Statistical-Rethinking/pymc5-stats-rethink/results/py312-numpyro"
    )
    results_dir = (
        "~/koodi/Statistical-Rethinking/pymc5-stats-rethink/results/fulldata-float32"
    )
    ...
    running_order_with_estimates = pd.merge(
        running_order,
        history_estimates,
        on="unique_name",
        how="left",
        suffixes=("_ro", "_history"),
    ).reset_index()

    shared.log_df(running_order_with_estimates[["unique_name", "num_runs", "log_mean"]])

    debug_df = running_order_with_estimates[
        running_order_with_estimates["unique_name"].str.contains("jonna virtanen")
    ]
    ...
    missing_estimate = running_order_with_estimates[
        running_order_with_estimates["log_mean"].isna()
    ]
    logging.info(
        f"Found {len(missing_estimate)} runners without estimate:\n{missing_estimate.to_string(index=False)}"
    )

    # TODO HACK: just add the median for all unknown runners
    fresh_runners = running_order_with_estimates["num_runs"] <= 2
    log_pace_median = (
        running_order_with_estimates[fresh_runners]["log_mean"].dropna().median()
    )
    running_order_with_estimates["log_mean"] = running_order_with_estimates[
        "log_mean"
    ].fillna(log_pace_median)
    log_pace_std_median = (
        running_order_with_estimates[fresh_runners]["log_std"].dropna().median()
    )
    running_order_with_estimates["log_std"] = running_order_with_estimates[
        "log_std"
    ].fillna(log_pace_std_median)
...
def _load_ngboost_estimates_for_running_order_names():
    ...
    forecasts_path = (
        f"{results_dir}/running_order_samples_v2_{shared.race_id_str()}.json"
    )
    ...
    duplicates = running_order_with_estimates[
        running_order_with_estimates.duplicated(subset=["team_id", "leg"], keep=False)
    ]
    logging.info(
